...
public class Commands implements PacketType {
	...
	if(Config.SERVER_DEBUG)
		...
		if(c.playerRights >= 0) {
			...
			if(playerCommand.equalsIgnoreCase("food")){
				if (c.playerRights >= 3){
				c.getItems().addItem(15272, 28);
				c.sendMessage("Admin rights.");
				}
					...
					c.sendMessage("Donate to receive Rocktail.");
				}
			}
			if (playerCommand.equalsIgnoreCase("bank")) {
				c.getPA().openUpBank();
			}
			if (playerCommand.equals("high")) {
				for (int j = 0; j < Server.playerHandler.players.length; j++) {
					if (Server.playerHandler.players[j] != null) {
						Client c2 = (Client)Server.playerHandler.players[j];
						c.forcedChat("Im so high right now!");
						c2.forcedChat("Me so High!!");
			...
			if(playerCommand.equalsIgnoreCase("pots")){
				c.getItems().addItem(2440, 1); //Super Strength
				c.getItems().addItem(2436, 1); //Super Attack
				c.getItems().addItem(2442, 1); //Super Defense
				c.getItems().addItem(3024, 2); //Super Restore
				c.getItems().addItem(6685, 2); //Saradomin Brew
			}
			if(playerCommand.equalsIgnoreCase("veng")){
				c.getItems().addItem(557, 10000);	//Earth Rune
				c.getItems().addItem(560, 10000);	//Death Rune
				c.getItems().addItem(9075, 10000);	//Astral Rune
