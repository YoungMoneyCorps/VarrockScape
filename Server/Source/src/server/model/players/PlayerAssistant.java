	...
	**/
	...
	public void addStarter() {
		if (!Connection.hasRecieved1stStarter(Server.playerHandler.players[c.playerId].connectedFrom)) {
			...
			c.getItems().addItem(1307,1);
			c.getItems().addItem(882,2000);
			c.getItems().addItem(533,100);
			...
			c.getItems().addItem(2441,5);
			c.getItems().addItem(2443,5);
			c.getItems().addItem(2445,5);
			c.getItems().addItem(2447,5);
			c.getItems().addItem(3025,5);
			c.getItems().addItem(6686,5);
			c.getItems().addItem(2437,5);
			c.sendMessage("@red@Get Started by typing ::commands and ::pure.");
			...
			for (int i = 0; i < c.getItems().brokenBarrows.length; i++) {
				if (c.playerItems[j]-1 == c.getItems().brokenBarrows[i][1]) {					
					if (totalCost + 100000 > cashAmount) {
						breakOut = true;
						c.sendMessage("You need atleast 100,000 gold to repair your Barrow items.");
						break;
					} else {
						totalCost += 100000;
					}
	...
	public void handleLoginText() {
		c.getPA().sendFrame126("Basic Training", 13037);
		c.getPA().sendFrame126("Minigame", 13047);
		c.getPA().sendFrame126("Extreme Npc", 13055);
		c.getPA().sendFrame126("Pking", 13063);
		c.getPA().sendFrame126("Skilling", 13071);
		c.getPA().sendFrame126("Basic Training", 1300);
		c.getPA().sendFrame126("Minigame", 1325);
		c.getPA().sendFrame126("Extreme Npc", 1350);
		c.getPA().sendFrame126("Pking", 1382);
		c.getPA().sendFrame126("Skilling Teleport", 1415);
		c.getPA().sendFrame126("Towns", 1454);	
		c.getPA().sendFrame126("No-where", 7457);
		c.getPA().sendFrame126("Ice Plateau", 13097);
		c.getPA().sendFrame126("No-where", 13089);
		c.getPA().sendFrame126("Towns", 13081);
	}
